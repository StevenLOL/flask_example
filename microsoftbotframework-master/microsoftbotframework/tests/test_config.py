...
class ConfigTestCase(TestCase):
    ...
    def test_yaml_load(self):
        yaml_config = self.config._get_yaml_config(
            os.getcwd() + '/microsoftbotframework/tests/test_files/good_config.yaml')
        self.assertEqual(yaml_config,
                         {'other': {'app_client_id': '245gsh246hjb', 'app_client_secret': '45hrtvb24hrtwhwrtb'},
    ...
    def test_missing_yaml(self):
        yaml_data = self.config._get_yaml_config(
            os.getcwd() + '/microsoftbotframework/tests/test_files/fjipowgnaoepirvniurv.yaml')
        self.assertEqual(yaml_data, {})

    def test_yaml_bad_formatting(self):
        with self.assertRaises(Exception):
            yaml_data = self.config._get_yaml_config(
                os.getcwd() + '/microsoftbotframework/tests/test_files/no_roots.yaml')

    # Test yaml merge
    def test_yaml_merge_same_values(self):
        default_values = {'other': {'https_proxy': None, 'http_proxy': None}, 'celery': {'broker_url': None}}
        yaml_config = {'other': {'https_proxy': '245gsh246hjb', 'http_proxy': '45hrtvb24hrtwhwrtb'},
                       'celery': {'broker_url': 'None'}}
        result = {'other': {'https_proxy': '245gsh246hjb', 'http_proxy': '45hrtvb24hrtwhwrtb'},
                  'celery': {'broker_url': 'None'}}
